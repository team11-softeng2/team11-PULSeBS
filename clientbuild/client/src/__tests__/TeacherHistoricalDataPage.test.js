...
test('getDataFromServerAndUpdate', async () => {
  ...
  expect(instance.updateGraphData).toHaveBeenCalledTimes(1);
...
test('updateGraphData', async () => {
  ...
  await instance.updateGraphData(seriesMock, seriesMock, ["test"], "test", "test", "test");
  expect(instance.state.series).toStrictEqual(expectedRes);
});
