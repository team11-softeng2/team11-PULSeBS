...
test("render Dropzone with file", async () => {
    const tree = TestRenderer.create(<Dropzone disabled={false} onFileAdded={jest.fn()} file={{type: "text/csv", name: "test"}}/>).toJSON();
    expect(tree).toMatchSnapshot();
});
...
test("onDrop", async () => {
    ...
    await instance.onDrop({dataTransfer: {files: [{type: "text/csv"}, {type: "text/csv"}]}, preventDefault: jest.fn()});
    expect(instance.props.onFileAdded).toHaveBeenCalledTimes(1);
    expect(instance.state.highlight).toBe(false);
    await instance.onDrop({dataTransfer: {files: [{type: "text/JSON"}]}, preventDefault: jest.fn()});
    expect(instance.props.onFileAdded).toHaveBeenCalledTimes(1);
    expect(instance.state.highlight).toBe(false);
...
test("onDragLeave", async () => {
    ...
    const instance = await component.instance();
    await instance.onDragLeave({dataTransfer: {files: [{type: "text/csv"}]}, preventDefault: jest.fn()});
    expect(instance.state.highlight).toBe(false);
});
...
test("onDragOver", async () => {
    ...
    const instance = await component.instance();
    expect(instance.state.highlight).toBe(false);
    await instance.onDragOver({target: {files: [{type: "text/csv"}]}, preventDefault: jest.fn()});
...
test("onFileAdded", async () => {
    ...
    const instance = await component.instance();
    expect(instance.state.highlight).toBe(false);
    await instance.onFileAdded({target: {files: [{type: "text/csv"}]}, preventDefault: jest.fn()});
    expect(instance.props.onFileAdded).toHaveBeenCalledTimes(1);
...
test("openFileDialog", async () => {
    ...
    await instance.openFileDialog({target: {files: [{type: "text/csv"}]}, preventDefault: jest.fn()});
